...
export default function bannerFormView() {
    ...
    async function salvar() {
        try {
            if (!id) {
                await bannerService.salvar({ titulo, imagem, link, ordem: Number(ordem), ativo });
            } else {
                await bannerService.atualizar(id, { titulo, imagem, link, ordem: Number(ordem), ativo });
            }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.form}>
                ...
                <TextInput value={titulo} onChangeText={setTitulo} mode="outlined" style={styles.input} />

                <Text style={styles.label}>Imagem</Text>
                <Button mode="outlined" onPress={escolherImagem}>Selecionar</Button>
                {imagem && <Image source={{ uri: imagem }} style={styles.preview} />}

                <Text style={styles.label}>Link (opcional)</Text>
                <TextInput value={link} onChangeText={setLink} mode="outlined" style={styles.input} />

                <Text style={styles.label}>Ordem</Text>
                <TextInput value={ordem} onChangeText={setOrdem} mode="outlined" keyboardType="numeric" style={styles.input} />
                ...
                <Button mode="contained" style={styles.btnSave} onPress={salvar}>Salvar</Button>
            </View>
        </View>
    );
}
